...
export default function ActionBar({ onEdit, onDelete, onClose }: Props) {
  return (
    <View pointerEvents="box-none" className="absolute left-4 right-4 bottom-3 items-center">
      <View className="bg-white w-full rounded-lg p-2 flex-row justify-center  gap-10 items-center " style={{ elevation: 6 }}>
        <Pressable onPress={() => { onEdit?.(); onClose(); }} className="p-2 mt-1 rounded-md items-center">
          <Edit3 size={20} color="#111827" />
          <Text className="text-xs mt-1">Edit</Text>
        </Pressable>

        <Pressable onPress={() => { onDelete?.(); onClose(); }} className="p-2 mt-1 rounded-md items-center">
